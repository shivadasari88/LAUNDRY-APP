        ...
        <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                ...
                <div className="bg-indigo-900/40 p-5 rounded-xl border border-indigo-500/30 shadow-lg">
                    <h3 className="text-3xl font-bold text-indigo-300">{orders.filter(o => o.status === 'PROCESSING').length}</h3>
                    <p className="text-indigo-200/70 text-sm">Processing</p>
                </div>
                <div className="bg-emerald-900/40 p-5 rounded-xl border border-emerald-500/30 shadow-lg">
                    ...
                    <p className="text-emerald-200/70 text-sm">Ready to Deliver</p>
                </div>
                <div className="bg-purple-900/40 p-5 rounded-xl border border-purple-500/30 shadow-lg">
                    <h3 className="text-3xl font-bold text-purple-300">{orders.filter(o => o.status === 'DELIVERED').length}</h3>
                            ...
                            {orders.length === 0 ? (
                                ...
                                orders.map((order) => (
                                    <tr key={order.id} className="hover:bg-slate-700/30 transition-colors">
                                        <td className="px-6 py-4 font-mono text-white">#{order.orderDisplayId || order.id}</td>
                                        <td className="px-6 py-4">
                                            <div className="font-bold text-white">{order.customerName}</div>
                                            <div className="text-xs text-slate-500">{new Date(order.orderTime).toLocaleDateString()}</div>
                                        </td>
                                        <td className="px-6 py-4">{order.itemsDescription}</td>
